import { getNamedType, isEnumType, isInputObjectType, isInterfaceType, isListType, isNonNullType, isObjectType, isUnionType, isWrappingType, parseType, print, typeFromAST, TypeInfo, visit, visitWithTypeInfo, } from 'graphql';
...
export function typeScriptSignatureForOperationVariables(variableNames, schema, operationDefinition) {
...
export function typeScriptDefinitionObjectForOperation(schema, operationDefinition, fragmentDefinitions, shouldLog = true) {
    let scalarMap = {
        String: 'string',
        ID: 'string',
        Int: 'number',
    ...
    let helper = (parentGqlType, selection) => {
        ...
        let isList = isListType(gqlType) || (!isNullable && isListType(gqlType.ofType));
        let isObjectLike = isObjectType(namedType) ||
    ...
    let baseGqlType = operationDefinition.kind === 'OperationDefinition'
...
export function typeScriptForOperation(schema, operationDefinition, fragmentDefinitions) {
    let typeMap = typeScriptDefinitionObjectForOperation(schema, operationDefinition, fragmentDefinitions);
...
export function typeScriptSignatureForOperation(schema, operationDefinition, fragmentDefinitions) {
    let types = typeScriptForOperation(schema, operationDefinition, fragmentDefinitions);
